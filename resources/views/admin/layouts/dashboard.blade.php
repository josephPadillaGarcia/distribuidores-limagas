                ...
                <li class="nav-item">
                    ...
                    <b-collapse id="{{$module["variable"]}}" v-cloak>
                        <ul class="nav flex-column sub-menu">
                            ...
                            <li class="nav-item">
                                <a class="nav-link  {{ Request::path() ==  $submodule["slug"] ? 'active' : ''   }}" href="/admin/{{ $submodule["slug"] }}">{{ $submodule["name"] }}</a>
                            </li>
                            @endforeach
                        </ul>
                    </b-collapse>

                    <a class="nav-link position-relative text-center d-inline-block w-100 py-3 nav-link--hover {{ Request::segment(2) ==  $module["variable"] ? 'active' : ''   }}" :id="'popover-{{$module["variable"]}}'">
                    ...
                    <b-popover :target="'popover-{{$module["variable"]}}'" custom-class="ml-0 shadow-none pl-0 pt-1 pb-2" triggers="hover" placement="right" v-cloak>
                        ...
                        <ul class="nav flex-column sub-menu">
                            @foreach($module["menu_secondary"] as $submodule)
                            <li class="nav-item">
                                <a class="nav-link py-1 px-0" href="/admin/{{ $submodule["slug"] }}">{{ $submodule["name"] }}</a>
                            </li>
                            @endforeach
                        </ul>
                    </b-popover>
                    @else
                    <a class="nav-link position-relative d-inline-block w-100 py-3 {{  Request::segment(2) ==  $module["variable"] ? 'active' : '' }}" href="/admin/{{ $module["slug"] }}">
                        <span class="current-color mr-3  ri-lg ri-{{$module["icon"]}}-line"></span>
                        <span class="nav-link__text">{{ $module["name"] }}</span>
                    </a>
                    <a class="nav-link position-relative text-center d-inline-block w-100 py-3 nav-link--hover {{  Request::segment(2) ==  $module["variable"] ? 'active' : '' }}" :id="'popover-{{$module["slug"]}}'">
                        <span class="current-color mr-3  ri-lg ri-{{$module["icon"]}}-line"></span>
                    </a>
                    <b-popover :target="'popover-{{$module["slug"]}}'" custom-class="ml-0 shadow-none pl-0 pr-5" triggers="hover" placement="right" v-cloak>
                        <a class="nav-link pt-1 px-0" href="/admin/{{ $module["slug"] }}">
